...
```{r first-look}
runs_dir <- here::here("output/runs/convnet")

runs <- ingest_runs(runs_dir) %>%
  filter(completed, !is.na(rmse)) %>%
  arrange(rmse)
...
runs %>%
  ...
  select(run_dir, flag_res, rmse, flag_batch_size, flag_lr, epochs, flag_callback_reduce_lr) %>%
