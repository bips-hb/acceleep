...
cv_results %>%
  ...
  theme_ipsum(base_family = "Source Sans 3", grid = "Y")
...
cv_results %>%
  ...
  theme_ipsum(base_family = "Source Sans 3", grid = "X")
```
...
```{r cnn-results-boxplot-best-derivs}
# By accelerometer (flipped)
cv_results_full %>%
  filter(stringr::str_detect(model_spec, "CF256K20\\-MP10\\-CF128K10") | timestamp == 20200910115426) %>%
  tidyr::unnest(data) %>%
  ggplot(aes(y = reorder(model_spec, -rmse), x = rmse)) +
  facet_grid(rows = vars(accel), cols = vars(outcome_unit), scales = "free") +
  geom_point(size = 1, alpha = .2, position = position_jitter(width = .15, seed = 23)) +
...
```{r cnn-results-ranked-best-derivs}
cv_results_full %>%
  filter(stringr::str_detect(model_spec, "CF256K20\\-MP10\\-CF128K10") | timestamp == 20200910115426) %>%
  tidyr::unnest(data) %>%
  group_by(model_spec, timestamp, outcome_unit) %>%
